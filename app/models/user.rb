class User < ApplicationRecord
  ...
  validates :name, presence: true, unless: :account?
  ...
  # Check if this user has an associated account
  def account?
    account.present?
  end

  # Display name for the user (name or email from account)
  def display_name
    name.present? ? name : account&.email
  end
end
